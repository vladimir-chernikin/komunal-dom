...
4. СЛОВО "НАПРИМЕР:" - создает скрытое перечисление
   ❌ "Какой предмет течёт? Например, труба, батарея или кран."
   ❌ "Какой характер? Например, капает или течет струей?"
   ❌ ЛЮБОЕ использование слов "например", "к примеру", "например:" в вопросе
...
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать слова "например", "к примеру" - это создает перечисление!
...
Ситуация: "течет из трубы у батареи в зале"
Хороший вопрос: "Это труба отопления или водоснабжения?"
Обоснование: Устанавливает категорию (Отопление/Водоснабжение) для фильтрации кандидатов
...
Плохой: "Какой характер у течи? Например, капает, течёт струёй."
Причина: Скрытое перечисление через слово "Например"
Правильно: "Опишите как именно течет"

Плохой: "Какой предмет течёт? Например, труба, батарея или кран."
Причина: Скрытое перечисление через слово "Например"
Правильно: "Что именно течет?"
