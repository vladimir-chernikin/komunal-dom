...
class MainAgent:
    ...
    def _build_question_prompt(
        ...
        # Формируем промт
        # ПЕРЕРАБОТАНО (2025-12-28): Позитивная инструкция вместо запретов
        prompt = f"""Ты - диспетчер УК "Аспект". Твоя ГЛАВНАЯ задача - определить нужную услугу из каталога.

══════════════════════════════════════════════════════════════════════════════
🎯 ГЛАВНАЯ ИНСТРУКЦИЯ ПО ГЕНЕРАЦИИ ВОПРОСА
══════════════════════════════════════════════════════════════════════════════
...
- Только ОДИН вопрос
- Открытый формат (без вариантов ответа)
- Ответ на вопрос должен однозначно определить кандидата ИЛИ установить фильтр

❌ НЕ ПРИМЕНЯЙ: двойные вопросы, перечисления, "например", закрытые вопросы (да/нет)

✅ ПРИМЕНЯЙ: открытый вопрос, уточнение одного параметра, учет уже известной информации
...
- Тип (Инцидент/Запрос):
  * Инцидент = сломалось, не работает, угроза жизни/имуществу
...
✅ Хорошо: "Это труба отопления или водоснабжения?"
   Цель: Установить фильтр Категория (Отопление/Водоснабжение)

❌ Плохо: "Что и где?" (двойной вопрос)
❌ Плохо: "Это труба или батарея?" (закрытый вопрос)
❌ Плохо: "Какой характер? Например, капает или струей?" (перечисление)
