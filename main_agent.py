...
- Плохо: "Какая система: водоснабжение, отопление или канализация?"
- Хорошо: "Какая система повреждена?"
...
Верни ТОЛЬКО текст вопроса (без кавычек и пояснений)."""
    ...
    async def _llm_validate_question(
        ...
        # РЕШЕНИЕ: При обнаружении "или" всегда возвращаем простой вопрос без перечисления
        if re.search(r'\s+или\s+', question_lower):
            ...
            if not re.search(r'(оправить|измерить|прось|близ)', question_lower):
                ...
                # ИСПРАВЛЕНИЕ (2026-01-11): Всегда возвращаем простой открытый вопрос
                # НЕ пытаемся угадать контекст - просто спрашиваем "Опишите подробнее"
                logger.info("Исправляем на общий открытый вопрос БЕЗ перечисления")
                return "Опишите подробнее, что именно произошло?"
...
"""
            ...
            task_block = f"""
БЛОК: СПИСОК КАНДИДАТОВ ИЗ КАТАЛОГА УСЛУГ
{candidates_json}

ПОЛЯ КАТАЛОГА УСЛУГ:
- id: уникальный идентификатор услуги
- name: название услуги
- category: категория (Водоснабжение, Отопление, Электричество и т.д.)
- location: тип локации (Индивидуальное - в квартире, Общедомовое - дом/подъезд)

БЛОК: ЗАДАЧА
Кандидатов мало (2-10 штук). Найди критическое отличие между ними по ПАРАМЕТРАМ КАТАЛОГА
(объект проблемы или место) и задай уточняющий вопрос.
НЕ спрашивай о том, что УЖЕ есть в блоке "АБСОЛЮТНЫЕ ФАКТЫ (ЗАПРЕТ НА ВОПРОСЫ)".
"""
        elif strategy == 'C':
            ...
            # ИСПРАВЛЕНИЕ (2026-01-11): Усилена формулировка - категорический запрет на перечисление
            analysis_block = """
...
"""
            ...
            # ИСПРАВЛЕНИЕ (2026-01-11): Категорический запрет на использование перечислений в вопросе
            if categories and len(categories) > 1:
                analysis_block += f"""

⛔⛔⛔ КРИТИЧЕСКИ ВАЖНО - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО ПЕРЕЧИСЛЯТЬ ВАРИАНТЫ! ⛔⛔⛔
В блоке "РАЗНЫЕ КАТЕГОРИИ" перечислены: {', '.join(categories)}
Ни в коем случае НЕ включай эти категории в вопрос через запятую или "или"!
ПЛОХО: "Какая система: водоснабжение, отопление или канализация?"
ХОРОШО: "Какая система повреждена?" или "К какому типу относится проблема?"

СПРАШИВАЙ ТОЛЬКО: "Какая категория?" или "К какой системе относится?" БЕЗ перечисления вариантов!
"""
