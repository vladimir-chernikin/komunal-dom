    ...
    async def process_service_detection(self, message_text: str, user_context: Dict = None) -> Dict:
        ...
        try:
            ...
            if orch_result.get('status') == 'SUCCESS':
                ...
                # ИСПРАВЛЕНО (2026-02-24): Хардкод location_known УДАЛЁН - AI Orchestrator сам решает, нужно ли уточнять локацию

                # Создаем заявку (SUCCESS)
                result = {
                    'status': 'SUCCESS',
                    'service_id': orch_result.get('service_id'),
                    'service_name': orch_result.get('service_name'),
...
Верни ТОЛЬКО текст вопроса (без кавычек и пояснений)."""
    ...
    async def _llm_validate_question(
        ...
        try:
            ...
            # Fallback-промпт (захардкожен)
            # ИСПРАВЛЕНО (2026-03-05): Убраны примеры fixed_question чтобы LLM не возвращал их буквально!
            prompt = f"""Ты - строгий валидатор вопроса AI-диспетчера.
...
1. ПРОВЕРКА ЧТО ЭТО ВОПРОС:
   Текст ДОЛЖЕН быть вопросом (заканчиваться на "?").

   ПРИМЕРЫ НЕВОПРОСОВ (ОТКЛОНИТЬ):
   - "Нет необходимости уточнять локацию." → НЕ ВОПРОС
   - "Я понял проблему." → НЕ ВОПРОС

   ПРИМЕРЫ ВОПРОСОВ (ПРИНЯТЬ):
   - "Где именно это произошло?" → ВОПРОС
   - "Что именно сломалось?" → ВОПРОС

2. ПРОВЕРКА НА ИЗБЫТОЧНОСТЬ:
   Вопрос ИЗБЫТОЧЕН, если спрашивает о том, что УЖЕ есть в "Известных фактах".

   ПРИМЕРЫ ИЗБЫТОЧНЫХ:
   - Факт: "location=зал", вопрос: "Где именно?" → ИЗБЫТОЧЕН
   - Факт: "Проблема: течет", вопрос: "Что случилось?" → ИЗБЫТОЧЕН

3. ПРОВЕРКА НА ДВОЙНОЙ ВОПРОС:
...
ВАЖНО: Если вопрос НЕ прошел валидацию, верни valid=false и опиши причину в reason.
НЕ возвращай fixed_question - вместо этого верни valid=false с четким описанием проблемы.

Если вопрос прошел валидацию, верни valid=true с пустым reason.
        ...
        try:
            ...
            if '{' in response:
                ...
                if result.get('valid'):
                    ...
                    # ИСПРАВЛЕНО (2026-03-05): Если валидация не прошла - добавляем в txtStopQ и возвращаем исходный вопрос
                    # MainAgent сам перегенерирует вопрос с учетом txtStopQ
                    ...
                    if txtStopQ is not None:
                        txtStopQ.append(question)
                        logger.warning(f"❌ ДОБАВЛЕНО В txtStopQ (валидация): '{question[:50]}...'")
                        logger.warning(f"❌ Размер txtStopQ: {len(txtStopQ)} вопросов")
...
"""
                ...
                # Первый раз - задаем уточняющий вопрос
                # ИСПРАВЛЕНО (2026-02-18): Убрано "Похоже на..." - избыточно
                logger.info(f"[DEBUG] Первый уточняющий вопрос (без 'Похоже на...')")
                task_block = f"""
БЛОК: ЗАДАЧА
Услуга определена: {candidate_name}.
...
"""
    ...
    async def _build_question_prompt(
        ...
        # ИСПРАВЛЕНО (2026-02-05): Загружаем промпт из БД
        # ИСПРАВЛЕНО (2026-02-18): Убран fallback-хардкод - промпт ДОЛЖЕН быть в БД!
        try:
            from llm_tester.models import PromptTemplate
            from asgiref.sync import sync_to_async
            ...
            if not db_template:
                # ИСПРАВЛЕНО (2026-02-18): Прямая ошибка с инструкцией - без fallback!
                logger.error("[DB] Промпт 'mainagent-orchestrator' не найден в БД!")
                raise Exception(
                    "КРИТИЧЕСКАЯ ОШИБКА: Промпт 'mainagent-orchestrator' не найден в БД!\n"
                    "Создайте промпт в админке: /admin-uk/llm_tester/prompttemplate/\n"
                    "Slug: 'mainagent-orchestrator'\n"
                    "Is Active: True\n"
                    "Template: скопируйте из бэкапа или создайте новый"
                )

            # Подставляем переменные в базовую часть шаблона из БД
            # ИСПРАВЛЕНО (2026-02-18): Добавлены все плейсхолдеры с пустыми значениями
            # (реальные данные добавляются ниже через prompt +=)
            # ИСПРАВЛЕНО (2026-02-18): Добавлены KNOWLEDGE_BASE и SERVICE_CATALOG для RAG-промпта
            prompt = db_template.template.format(
                absolute_facts="",       # Плейсхолдер из БД (заполняется ниже через +=)
                txtPrb="",               # Плейсхолдер из БД (заполняется ниже через +=)
                dialog_history="",       # Плейсхолдер из БД (заполняется ниже через +=)
                established_filters="",  # Плейсхолдер из БД (заполняется ниже через +=)
                candidates="",           # Плейсхолдер из БД (заполняется ниже через +=)
                context=context,         # context используется для формирования промпта
                KNOWLEDGE_BASE="",       # RAG: База знаний (пока пусто, можно заполнить позже)
                SERVICE_CATALOG=""       # RAG: Каталог услуг (пока пусто, можно заполнить позже)
            )

            logger.debug(f"[DB] Промпт mainagent-orchestrator загружен из БД (ID: {db_template.id})")

        except Exception as e:
            # ИСПРАВЛЕНО (2026-02-18): Не скрываем ошибку, поднимаем exception
            logger.error(f"[DB] КРИТИЧЕСКАЯ ОШИБКА загрузки промпта из БД: {e}", exc_info=True)
            raise  # ← Падаем сразу, без fallback!
