...
class CompanyDepartmentAdmin(admin.ModelAdmin):
    ...
    list_display = ['department_tree', 'department_code', 'is_active']
    list_filter = []
    ...
    fieldsets = (
        ('Основное', {
            'fields': ('company_context', 'parent_department', 'department_name', 'department_code', 'is_active')
        }),
    )
    ...
    def get_fieldsets(self, request, obj=None):
        if obj is None:
            return (
                ('Основное', {
                    'fields': ('parent_department', 'department_name', 'department_code', 'is_active')
                }),
            )
...
class CompanyRouteMappingAdmin(admin.ModelAdmin):
    ...
    ordering = ['company__name', 'route__route_code', 'route__id']
    autocomplete_fields = ['company', 'route']
    ...
    def get_queryset(self, request):
        queryset = super().get_queryset(request).select_related('company', 'route', 'target_department')
        scoped_company = self._get_scoped_company(request)
    ...
    def fill_by_company_view(self, request):
        ...
        routes = list(RouteRef.objects.filter(is_active=True).order_by('route_code', 'route_name', 'id'))
        mappings = {
            mapping.route_id: mapping
            for mapping in CompanyRouteMapping.objects
            .filter(company=scoped_company)
            .select_related('route', 'target_department')
        }
        ...
        if request.method == 'POST':
            ...
            for route in routes:
                mapping = mappings.get(route.pk)
                raw_department_id = (request.POST.get(f'target_department_{route.pk}') or '').strip()
                is_active = request.POST.get(f'is_active_{route.pk}') == 'on'
                ...
                try:
                    ...
                    errors.append(f'Некорректное подразделение для маршрута {route}.')
                    continue

                if department_id not in department_ids:
                    errors.append(f'Подразделение для маршрута {route} не принадлежит выбранной компании.')
                    continue
            ...
            if errors:
                ...
                self.message_user(
                    request,
                    f'Маппинг по компании сохранен. Создано: {len(to_create)}, обновлено: {len(to_update)}.',
        ...
        for route in routes:
            mapping = mappings.get(route.pk)
            posted_department_id = request.POST.get(f'target_department_{route.pk}') if posted else None
            ...
            rows.append({
                ...
                'is_active': (
                    request.POST.get(f'is_active_{route.pk}') == 'on'
        ...
        context = {
            **self.admin_site.each_context(request),
            ...
            'title': f'Заполнить маппинг маршрутов: {scoped_company.name}',
...
class SLAPolicyAdmin(admin.ModelAdmin):
    ...
    list_filter = ['company', 'service', 'calendar_type', 'is_active']
