...
class ChatIntakeService:
    "Создает промежуточный JSON intake и реальную заявку из чат-диалога."
        ...
        *,
        ...
        if existing_work_order_id:
            ...
            if existing:
                return {
                    "created": True,
                    "request_intake_id": existing.request_intake_id,
        ...
        with transaction.atomic():
            request_intake = RequestIntake.objects.create(
                company=company,
                channel_code=self.payload_builder.CHANNEL_MAP.get(channel, "site_chat"),
            ...
            WorkOrderEventLog.objects.create(
                ...
                event_payload_json={
                    "request_intake_id": request_intake.id,
                    "normalized_payload_json": payload,
                },
