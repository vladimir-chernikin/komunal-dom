...
## ⚠️⚠️⚠️ 2. GITHUB - ОБЯЗАТЕЛЬНЫЕ КОММИТЫ ⚠️⚠️⚠️

### 🔥 КРИТИЧЕСКИ ВАЖНО: GitHub - ПЕРВЫЙ ДЕЙСТВИЕ ПРИ ЛЮБЫХ ИЗМЕНЕНИЯХ

**ПРАВИЛО:** КАЖДОЕ изменение в коде НЕМЕДЛЕННО коммитится и пушится в GitHub!

```
Изменил код → git add → git commit → git push → (только потом) продолжай работу
```

**НЕВЫПОЛНЕНИЕ ПРИВЕДЕТ К:**
- ❌ Потере работы при ошибках/сбоях
- ❌ Невозможности откатиться к рабочей версии
- ❌ Конфликтам при слиянии изменений
- ❌ Потере контекста что было изменено

**ПРОЦЕСС:**
1. ✅ **ДО изменений:** `git pull` (получить последние изменения)
2. ✅ **Изменение кода:** (.py, .html, .js файлы)
3. ✅ **НЕМЕДЛЕННО ПОСЛЕ изменений:**
   ```bash
   git add file1.py file2.py
   git commit -m "Описание изменений"
   git push
   ```
4. ✅ **Только потом:** Продолжай работу или переходи к следующей задаче
...
# 2. Вносим изменения (например, в tag_search_service.py)
# ... редактирование ...

# 3. НЕМЕДЛЕННО коммитим
git add tag_search_service.py
git commit -m "Добавлен триграммный поиск по тегам"
git push
...
- ❌ ЗАПРЕЩЕНО: Оставлять изменения незакоммиченными "на потом"
- ❌ ЗАПРЕЩЕНО: Делать несколько изменений перед коммитом
- ❌ ЗАПРЕЩЕНО: Откладывать коммит "потом разберусь"
- ✅ ОБЯЗАТЕЛЬНО: Коммитить и пушить немедленно после каждого изменения
- ✅ ОБЯЗАТЕЛЬНО: Создавать Pull Request для слияния в main

**Pull Request:**
```bash
# После push создаем PR
gh pr create --title "Описание изменений" --body "Детали..."
# Или через GitHub Web интерфейс
```

**ПРОВЕРКА ПЕРЕД КАЖДЫМ ИЗМЕНЕНИЕМ:**
- Я сделал git pull?
- Я закоммичу изменения СРАЗУ после редактирования?
- Я создам PR для слияния?
