...
## 4. GITHUB - ОБЯЗАТЕЛЬНЫЕ КОММИТЫ

**ПРОЦЕСС:**
1. ✅ **ДО изменений:** `git pull`
2. ✅ **ПОСЛЕ изменений:** `git add` → `git commit -m "описание"` → `git push`

**КАЖДЫЙ РАЗ при изменении `.py`, `.html`, `.js` файлов:**
- ❌ ЗАПРЕЩЕНО: Оставлять изменения незакоммиченными
- ✅ ОБЯЗАТЕЛЬНО: Коммитить и пушить немедленно
...
## 9. ПРАВА ДОСТУПА К ФАЙЛАМ В /TMP
...
**1. НЕ работать напрямую в `main`!** Вся разработка в отдельных ветках, изменения через Pull Request.

**2. Обязательный процесс:**
```bash
...
**3. Стандарты веток:** `feature/*`, `bugfix/*`, `hotfix/*`, `refactor/*`, `docs/*`, `test/*`

**4. Правильные коммиты:** "Добавить фильтрацию", "Исправить ошибку", "Обновить документацию"

**5. Чек-лист PR:** Код протестирован, нет отладки, форматирование соблюдено, описание написано

**Учетные данные:** `/var/www/komunal-dom_ru/CREDENTIALS.md` (в .gitignore)
