...
"""
Анализ работы VectorSearchService для сообщения "течет труба"
"""
...
async def test_vector_search():
    """Тестируем векторный поиск для фразы 'течет труба'"""
    ...
    print("=== АНАЛИЗ ВЕКТОРНОГО ПОИСКА ДЛЯ ФРАЗЫ 'ТЕЧЕТ ТРУБА' ===\n")

    # Показываем поисковые векторы для каждой услуги
    print("Поисковые векторы услуг:")
    for service_id, service_info in vector_search.service_cache.items():
        print(f"ID {service_id}: {service_info['name']}")
        print(f"  Поисковые термины: {service_info['search_terms']}")
        print(f"  Поисковый вектор: {service_info['search_vector']}")
        print()
