...
"""
Скрипт для создания 3 шаблонов промптов для FilterDetectionService
ИСПРАВЛЕНО (2026-01-20): Разбитие на 3 отдельных промпта
"""
...
{
  "incident_type": [значение incident_type],
  "confidence": [значение incident_confidence],
  "reasoning": [значение reasoning_txt]
}"""

# Промпт 2: location_type
...
Верни JSON в формате:
{
  "location_type": [значение location_type],
  "confidence": [значение location_confidence],
  "reasoning": [значение reasoning_txt]
}"""
...
templates_data = [
    {
        ...
        'description': 'Определение типа обращения (Инцидент/Запрос) через иерархию угроз',
    ...
    },
    {
        ...
        'prompt_type': 'filter_detection',
        'description': 'Определение типа локации (Индивидуальное/Общедомовое)',
        'template': location_type_template
    },
    {
        'name': 'FilterDetection: category',
        'slug': 'filter-category',
        'prompt_type': 'filter_detection',
        'description': 'Определение категории проблемы через матричную модель',
        'template': category_template
    }
]

print("Создаю шаблоны промптов...")
for tmpl_data in templates_data:
    # Проверяем, существует ли шаблон
    existing = PromptTemplate.objects.filter(slug=tmpl_data['slug']).first()
    if existing:
        print(f"✓ Шаблон '{tmpl_data['name']}' уже существует, пропускаем")
        continue

    # Создаем шаблон
    template = PromptTemplate.objects.create(**tmpl_data, is_active=True)
    print(f"✓ Создан шаблон: {template.name} (ID: {template.id})")

print("\nГотово! Теперь на странице /llm-tester/ будут доступны 3 шаблона:")
print("  1. FilterDetection: incident_type")
print("  2. FilterDetection: location_type")
print("  3. FilterDetection: category")
