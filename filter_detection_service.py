...
2. location_type (Индивидуальное или Общедомовое или null):
   ...
   ⛔⛔⛔ СУПЕР КРИТИЧЕСКОЕ ПРАВИЛО ДЛЯ location_type ⛔⛔⛔
   🚨🚨🚨 КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО устанавливать location_type по умолчанию! 🚨🚨🚨
   ...
   🎯 ПРИМЕРЫ (чтобы НЕ ошибаться):
   - "у меня капает" → location_type=null (НЕ УКАЗАНО где!)
   - "у меня течет" → location_type=null (НЕ УКАЗАНО где!)
   - "прорвало трубу" → location_type=null (НЕ УКАЗАНО где!)
   - "в квартире течет" → location_type="Индивидуальное" (ЯВНО указано!)
   - "на кухне капает" → location_type="Индивидуальное" (ЯВНО указано!)
   - "в подъезде течет" → location_type="Общедомовое" (ЯВНО указано!)
   - "на крыше протекает" → location_type="Общедомовое" (ЯВНО указано!)

   ⛔ ЕСЛИ НЕТ ЯВНОГО УКАЗАНИЯ МЕСТА → ВСЕГДА location_type=null!
   ⛔ ЛЮБОЕ НЕЯВНОЕ УКАЗАНИЕ → location_type=null!
...
⛔ ЗАПРЕЩЕНО: Возвращать несколько категорий через запятую!
   ...
   ⛔ СУПЕР КРИТИЧЕСКИ ВАЖНО - СЛОВА "ТЕЧЕТ/ПРОРЫВО/КАПАЕТ" БЕЗ КОНТЕКСТА:
   - "течет" / "прорывало" / "капает" / "протекает" БЕЗ указания системы → category=null!
   - "у меня течет" → НЕ ЗНАЕМ что именно (вода? канализация? отопление?) → category=null
   - "прорвало трубу" → НЕ ЗНАЕМ какая труба → category=null
   - "капает" → НЕ ЗНАЕМ что → category=null
   - Только ЯВНОЕ указание системы определяет category!
