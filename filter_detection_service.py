...
Инцидент (57 услуг):
- "Нет света во всем доме" → Инцидент
- "Прорыв труб в квартире" → Инцидент
- "Общедомовой прорыв труб" → Инцидент
- "Отсутствие отопления" → Инцидент
...
- "Установка и замена сантехники" → Запрос
- "Делопроизводство" → Запрос
- "Проведение общего собрания собственников" → Запрос
- "Проверка внутриквартирного электро оборудования" → Запрос
- "Технические характеристики и информация о состоянии дома" → Запрос
...
# 2.2 Угроза здоровью? (плесень/травма/электричество/озон/угарный газ)
если incident_confidence=="0.5" и последствия содержит ("плесень"/"травма"/"электричество"/"озон"/"угарный"):
    incident_type="Инцидент"; incident_confidence="0.9"
    reasoning_txt += " | 2.2: [" + последствия + "] → Инцидент(0.9)"

# 2.3 Угроза имуществу? (повреждение/затопление/отсутствие удобств)
# ПРИМЕР: "нет света/воды/тепла/газа" → отсутствие удобств → Инцидент
если incident_confidence=="0.5" и последствия содержит ("затоплен"/"поврежден"/"отсутств"/"нет свет"/"нет вод"/"нет тепл"/"нет газ"):
    incident_type="Инцидент"; incident_confidence="0.8"
    reasoning_txt += " | 2.3: [" + последствия + "] → Инцидент(0.8)"

# 2.4 Запрос без срочности? (если ни одно из условий выше не выполнилось)
