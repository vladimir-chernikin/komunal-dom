...
- incident_type: "Инцидент" - ЧТО-ТО СЛОМАЛОСЬ/ТЕЧЕТ/НЕ РАБОТАЕТ/ЗАСОРИЛО/ПРОРВАЛО (любая проблема, авария, поломка)
  ⚠️ КРИТИЧЕСКИ ВАЖНО: ТЕЧЕТ, ПРОРЫВ, ЗАТОПЛЕНИЕ = ВСЕГДА "Инцидент"! (это аварийная ситуация, вред имуществу!)
- incident_type: "Запрос" - ТОЛЬКО если пользователь ХОЧЕТ ИНФОРМАЦИЮ или спрашивает "как сделать", "почему", "когда"
  ⚠️ КРИТИЧЕСКИ ВАЖНО: Запрос не является проблемой! (спросить, узнать, уточнить = НЕ проблема)
...
⚠️⚠️⚠️ КРИТИЧЕСКИ ВАЖНО (2025-12-28):
1. ТЕЧЕТ ВОДЫ = "Инцидент" (аварийная ситуация!)
2. ПРОРЫВ ТРУБЫ = "Инцидент" (авария!)
3. ЗАТОПЛЕНИЕ = "Инцидент" (угроза имуществу!)
4. ЕСЛИ ЧТО-ТО ТЕЧЕТ/ЛАЕТ/ПРОРЫВАЛО = 100% "Инцидент", confidence: 1.0!

НЕ путай "описание проблемы" с "запросом информации"!
