...
1. incident_type (Инцидент или Запрос):
   "Инцидент" = АКТИВНАЯ ПРОБЛЕМА которая угрожает/наносит вред:
   - Поломка/разрушение (сломалось, лопнуло, прорвало, течет, капает, drip)
   - Блокировка/засор (не работает, забито, не уходит, засор, засорился)
   - УГРОЗА: риск жизни, здоровью, имуществу (затопление, замыкание, падение)
   - НЕИСПРАВНОСТЬ оборудования (отсутствие горячей воды, нет отопления)

   ⛔ КРИТИЧЕСКИ ВАЖНО - СЛОВА-МАРКЕРЫ ИНЦИДЕНТА:
   "течет" = ВСЕГДА Инцидент (100%!)
   "прорвало" = ВСЕГДА Инцидент (100%!)
   "капает" = ВСЕГДА Инцидент (100%!)
   "нет воды" = ВСЕГДА Инцидент (100%!)
   "засор" = ВСЕГДА Инцидент (100%!)
   "не работает" = ВСЕГДА Инцидент (100%!)

   "Запрос" = ХОЧЕТ ЧТО-ТО (не проблема):
   - Информацию (спросить, узнать, почему, когда, сколько стоит)
   - Дополнительную услугу (установить, заменить, подключить, монтаж, поставить)
   - Консультацию/разъяснение (объяснить, рассказать, уточнить)
