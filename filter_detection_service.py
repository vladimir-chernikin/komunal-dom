...
1. incident_type (Инцидент или Запрос):
   АНАЛИЗ ПОСЛЕДСТВИЙ: Определи что ПРОИЗОЙДЕТ если проблему не решат:
   ...
   ИСПРАВЛЕНИЕ (2026-01-06): КРИТИЧЕСКИ ВАЖНЫЕ ПРИМЕРЫ:
   - "течет" → Инцидент (последствие: ЗАТОПЛЕНИЕ, ущерб имуществу!)
   - "течет труба" → Инцидент (последствие: АВАРИЯ водоснабжения)
   - "прорвало трубу" → Инцидент (последствие: затопление)
   - "прорыв" → Инцидент (последствие: авария)
   - "засор" → Инцидент (последствие: блокировка канализации)
   - "не работает" → Инцидент (если это лифт/вода/газ/отопление)
   - "сломался" → Инцидент (если это критичное оборудование)
   - "установить" → Запрос (желание получить услугу)
   - "заменить" → Запрос (желание получить услугу)
   - "почему" → Запрос (желание получить информацию)
   - "когда" → Запрос (желание получить информацию)
