...
class FilterDetectionService:
    ...
    def _create_filter_detection_prompt(self, message_text: str, dialog_history: List[Dict], txtPrb: str = None) -> str:
        ...
        # ИСПРАВЛЕНО (2026-01-16): Правило "НЕ ПРИДУМЫВАЙ" перемещено в начало и усилено
        prompt = f"""# Классификатор обращений УК → фильтры каталога

⛔⛔⛔ КРИТИЧЕСКИ ВАЖНО: НЕ ПРИДУМЫВАЙ ДАННЫЕ! ⛔⛔⛔

Если TXT_PRB НЕ содержит явного упоминания объекта/локации →
КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО придумывать их из статистики!

ПРЯМЫЕ ЗАПРЕТЫ (нарушение строго наказуется):
- TXT_PRB = "у меня течет" → НЕТ "батарея", НЕТ "кран", НЕТ "внутри"! Верни OBJ=null, PLACE=null!
- TXT_PRB = "сломалось" → НЕТ "труба", НЕТ "розетка"! Верни OBJ=null!
- TXT_PRB = "в квартире" → Это локация, но НЕТ объекта! Верни OBJ=null, PLACE=квартира!

ПРАВИЛЬНЫЕ ПРИМЕРЫ:
✅ "течет батарея в зале" → OBJ=батарея, PLACE=зал, PLACE_SCOPE=внутри
✅ "у меня течет" → OBJ=null, PLACE=null, PLACE_SCOPE=null
✅ "сломался кран" → OBJ=кран, PLACE=null, PLACE_SCOPE=внутри
