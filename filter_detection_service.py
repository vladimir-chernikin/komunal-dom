...
2. location_type (Индивидуальное или Общедомовое или null):
   ...
   ⛔⛔⛔ СУПЕР КРИТИЧЕСКИ ВАЖНО - "У МЕНЯ" НЕ ЗНАЧИТ "В КВАРТИРЕ":
   - "у меня" БЕЗ уточнения места → null! (не угадывай!)
   - "у меня прорвало трубу" → location_type=null (может быть в квартире, в подъезде, где угодно!)
   - "у меня течет" → location_type=null (не указано где!)
   - "у меня сломалось" → location_type=null
   - "у меня проблема" → location_type=null

   ⛔ ЕСЛИ СКАЗАЛИ "У МЕНЯ" И ТОЛЬКО ПОТОМ "В КВАРТИРЕ" → location_type=Индивидуальное
   ⛔ ЕСЛИ СКАЗАЛИ ТОЛЬКО "В КВАРТИРЕ" → location_type=Индивидуальное
   ⛔ ЕСЛИ НЕ СКАЗАЛИ "В КВАРТИРЕ"/"В ПОДЪЕЗДЕ" → location_type=null!

   ПРИМЕРЫ ЯВНОГО УКАЗАНИЯ МЕСТА:
   - "у меня **в квартире** прорвало трубу" → Индивидуальное ✓
   - "**в квартире** прорвало трубу" → Индивидуальное ✓
   - "в **подъезде** не горит свет" → Общедомовое ✓
   - "на **крыше** течет" → Общедомовое ✓

   ПРИМЕРЫ НЕЯВНОГО (КАТЕГОРИЧЕСКИ location_type=null!):
   - "у меня прорвало трубу" → null
   - "у меня течет" → null
   - "сломалось" → null
   - "проблема у меня" → null
...
3. category (категория из списка выше):
   ...
   ⛔⛔⛔ СУПЕР КРИТИЧЕСКИ ВАЖНО - ТРУБЫ БЫВАЮТ РАЗНЫЕ:
   - "труба" БЕЗ контекста → category=null! (может быть водоснабжение/канализация/отопление!)
   - "прорвало трубу" → category=null! (неизвестно какая труба!)
   - "течет из трубы" → category=null! (неизвестно что в трубе!)
   - "у меня прорвало трубу" → category=null!
   - "у меня прорвало трубу в квартире" → category=null! (все еще неизвестно какая труба!)

   ПРИМЕРЫ ЯВНОГО УКАЗАНИЯ КАТЕГОРИИ:
   - "течет **вода** из крана" → Водоснабжение ✓
   - "течет **горячая вода** из трубы" → Отопление ✓
   - "течет **жидкость** с нечистотами" → Канализация ✓
   - "**батарея** холодная/не греет" → Отопление ✓
   - "не уходит **вода**/засор" → Канализация ✓
   - "**лифт** не ездит" → Лифты ✓

   ПРИМЕРЫ НЕЯВНОГО (КАТЕГОРИЧЕСКИ category=null!):
   - "прорвало трубу" → null (труба может быть любой!)
   - "у меня прорвало трубу" → null
   - "у меня прорвало трубу в квартире" → null (не указано какая труба!)
   - "у меня течет труба" → null
   - "течет" → null (неясно что)
   - "сломалось" → null (неясно что)
   - "проблема с трубой" → null

   ⛔ ЕСЛИ В ТЕКСТЕ ЕСТЬ СЛОВО "ТРУБА" НО НЕТ УКАЗАНИЯ ЧТО В НЕЙ (вода/газ/нечистоты) → category=null!
   ⛔ ЕСЛИ В ТЕКСТЕ ЕСТЬ СЛОВО "ТРУБА" И "КВАРТИРА" НО НЕТ УКАЗАНИЯ ЧТО В НЕЙ → category=null!
