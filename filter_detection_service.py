...
ИСПРАВЛЕНИЕ (2026-01-06): ПРИМЕРЫ ДЛЯ КРИТИЧЕСКИХ СЛУЧАЕВ:

КЛЮЧЕВЫЕ СЛОВА-МАРКЕРЫ (100% Инцидент):
- "течет", "капает", "льется", "протекает" → Инцидент (последствие: ЗАТОПЛЕНИЕ!)
- "прорвало", "лопнуло", "разорвало" → Инцидент (последствие: затопление/авария!)
- "засор", "забито", "не уходит" → Инцидент (последствие: блокировка канализации!)
- "нет воды", "нет света", "нет отопления" → Инцидент (последствие: БЛОКИРОВКА ресурса!)
- "затапливает", "заливает", "наводнение" → Инцидент (последствие: УЩЕРБ имуществу!)
- "авария", "поломка", "сломалось", "не работает" → Инцидент (активная проблема!)

КЛЮЧЕВЫЕ СЛОВА-МАРКЕРЫ (100% Запрос):
- "установить", "заменить", "поставить", "смонтировать" → Запрос (желание получить услугу)
- "спросить", "узнать", "почему", "когда", "как" → Запрос (желание получить информацию)
- "объяснить", "рассказать", "уточнить" → Запрос (консультация)

АНАЛИЗ ПОСЛЕДСТВИЙ (для неоднозначных случаев):
Определи что ПРОИЗОЙДЕТ если проблему не решат:

   ⛔ "Инцидент" = АКТИВНАЯ ПРОБЛЕМА с негативными последствиями:
   ...
   ✅ "Запрос" = БЕЗ негативных последствий:
   ...
   1. Каковы ПОСЛЕДСТВИЯ если не решать проблему?
   2. Есть ли УГРОЗА здоровью/имуществу?
   3. БЛОКИРУЕТСЯ ли доступ к жизненно важным ресурсам?
   4. Если ДА → Инцидент, если НЕТ → Запрос

   ИСПРАВЛЕНИЕ (2026-01-06): Используй ДВУХЭТАПНЫЙ анализ:
   ЭТАП 1: Определи последствия проблемы
   - "Если [описание проблемы] не решат, то произойдет: [последствия]"
   - Оцени: есть ли УГРОЗА здоровью/имуществу? БЛОКИРУЕТСЯ ли доступ к ресурсам?

   ЭТАП 2: Классифицируй по последствиям
   - Если последствия угрожают → Инцидент
   - Если без негативных последствий → Запрос
...
2. location_type (Индивидуальное или Общедомовое или null):
   ...
   - ИИ должен сам понимать смысл, НЕ используй хардкод списка помещений!
...
3. category (категория из списка выше):
   ...
   - "течет вода/жидкость из крана" → Водоснабжение
   - "не уходит вода/засор в трубе" → Канализация
   - "батарея холодная/не греет" → Отопление
   - "нет газа/пахнет газом" → Газоснабжение
   - "лифт не ездит/застрял" → Лифты

   ПРИМЕРЫ НЕЯВНОГО (НЕ УКАЗЫВАТЬ category!):
   - "прорвало трубу" → null (труба может быть любой!)
   - "течет" → null (неясно что)
   - "сломалось" → null (неясно что)

   КРИТИЧЕСКИ:
   - "труба" БЕЗ контекста → null! (может быть водоснабжение/канализация/отопление)
   - "течет" БЕЗ уточнения → null! (может быть вода/стоки/батарея)
   - Еслиambiguous → null!

4. object_description:
   - Краткое описание: "объект действие место" (макс 5 слов)
   - Примеры: "батарея течет кухня", "кран капает ванная"
...
НЕ ДОПУСКАЙ:
- Хардкода помещений ("ванная=квартира", "коридор=подъезд")
- Хардкода категорий ("труба=Водоснабжение")
- Списков синонимов - ИИ должен сам анализировать СМЫСЛ!
