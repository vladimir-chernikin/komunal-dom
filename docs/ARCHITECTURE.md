...
Агент работает по полезному принципу лабораторного ноутбука: Python выбирает
этап и его короткий промпт, GigaChat предлагает структурированные данные и
аргументы инструментов, Python исполняет инструменты и принимает переход.
Текстовые `[SIGNAL]` из ноутбука заменяются строгой JSON-схемой GigaChat.
Полный контракт находится в `AI_AGENT_RAG.md`.
...
Шаг 3 запускает промпт текущего этапа и только реально нужные независимые
задачи: адрес/УК, услуга/проблема, контакт, подтверждение/исправление.
Одновременно разрешено до пяти вызовов, но обычно выполняется один модельный
вызов и несколько инструментов. После проверки инструментами следует один
промпт решения и живого ответа. Только неразбираемый строгий JSON разрешает
один короткий повтор. Поэтому нормальный путь содержит не больше двух
последовательных стадий модели, аварийный — не больше трёх.
