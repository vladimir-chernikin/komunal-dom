...
На production создана отдельная PostgreSQL-БД `komunal_dom_b`. При начале этой
работы в ней было 0 прикладных таблиц. Полная схема ниже содержит 31 таблицу и
пока остаётся проектом. Первая отдельная миграция переносит только справочник
`request_types` — «ТипОбращения» — после проверки в отдельной задаче Codex.
