...
- legacy-контур `bot_service_requests` удален миграцией `work_orders.0012_refactor_ads_status_flow_and_drop_legacy_executor`

## Ключевой вывод

На 2026-04-07 после второго прохода по коду и БД:

1. legacy-таблицы `public.bot_service_requests` и `public.bot_service_requests_backup` удалены;
2. старые маршруты исполнителя `/executor/take|arrived|complete|upload-photo|report` сняты из `portal/urls.py`;
3. raw SQL-функции по `bot_service_requests` физически удалены из `portal/views.py`;
4. рабочим контуром заявок остается `request_mgmt.work_order` и связанные таблицы `work_orders`.
...
| `public.bot_service_requests` | - | `REMOVED` | Legacy-таблица старого кабинета исполнителя. Удалена миграцией `work_orders.0012_refactor_ads_status_flow_and_drop_legacy_executor`. |
| `public.bot_service_requests_backup` | - | `REMOVED` | Резервная копия legacy-таблицы заявок. Удалена той же миграцией вместе со старым контуром исполнителя. |
