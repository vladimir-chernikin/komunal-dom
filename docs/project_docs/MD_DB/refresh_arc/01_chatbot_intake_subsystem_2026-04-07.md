...
- Runtime no longer uses the intermediate `request_intake` model.
- After service confirmation the chat flow creates `work_order` directly through `ChatIntakeService.create_from_chat(...)`.
- Source payload, normalized payload, channel, `session_id`, `message_log_id` and `external_message_id` are stored in `work_order_event_log.event_payload_json` for the `created` event.
- `work_order` keeps only operational fields: `creation_source='bot_json'`, `message_log_ref`, resident user, object, service, route, department, status and request text.
- No extra fields from `request_intake` were moved into `work_order`: they are diagnostic intake data and belong in the event log, not in the main work order card.
