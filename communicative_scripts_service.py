...
class CommunicativeScriptsService:
    ...
    async def get_fallback_message(
        ...
        возвращается сообщение о передаче оператору.
        ...
        # Если ходов слишком много (>= 7), возвращаем финальное сообщение
        if dialog_turn >= 7:
            logger.warning(
                f"[ANTI-LOOP] Слишком много fallback сообщений (turn={dialog_turn}) -> "
                f"возвращаем финальное сообщение о передаче оператору"
            )
            return "К сожалению, я не смог определить вашу проблему. Пожалуйста, свяжитесь с оператором по телефону или опишите проблему другими словами."
